...
{% alert(type="note") %}
Если переменная перемещена, `drop` не вызывается для оригинала. Подробнее в [семантике ресурсов](@/tour/08-memory-management.ru.md#семантика-ресурсов-move-по-умолчанию).
{% end %}
