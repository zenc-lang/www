...
{% alert(type="note") %}
Структуры используют [Move Semantics](@/tour/08-memory-management.ru.md#семантика-ресурсов-move-по-умолчанию) по умолчанию. Доступ к полям можно получить через `.` даже на указателях (Auto-Dereference).
{% end %}
